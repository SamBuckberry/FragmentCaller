...
#'@importFrom signal specgram
#'@export
read.fsa <- function(path = "./", files = NULL, dye, SizeStandard,
  ...
  if(missing(Breaks)){
    ...
    Breaks <- read.fsa.bins(files = files, path = path, dye = dye,
                            SizeStandard = SizeStandard, Range = Range,
                            verbose = verbose, binwidth = binwidth, SNR = SNR)
  }
  if(verbose){
  ...
  Peaks <- do.call(rbind, lapply(seq_along(files), function(i){
    ...
    #only keeping the required information
    Selection <- c("DATA.1", "DATA.2", "DATA.3", "DATA.4")[c(pattern$Data$DyeN.1,
                                                             pattern$Data$DyeN.2,
                                                             pattern$Data$DyeN.3,
                                                             pattern$Data$DyeN.4)
                                                           %in% dye]
    ...
    Peak <- cumsum(abs(c(0, diff(pattern[, "DATA.105"] > quantile(
            pattern[,"DATA.105"], 1 - n / nrow(pattern))))))
    
    Index <- seq_len(
            nrow(pattern))[Peak %% 2 == 1][pattern[Peak %% 2 == 1,"DATA.105"] == ave(pattern[Peak %% 2 == 1, "DATA.105"], Peak[Peak %% 2 == 1], FUN = max)]
    Index <- Index[pattern[Index, "DATA.105"] <= SNR]
    while(length(Index) > length(SizeStandard)){
      n <- n - 1
      Peak <- cumsum(abs(c(0, diff(pattern[, "DATA.105"] > quantile(pattern[, "DATA.105"], 1 - n / nrow(pattern))))))
